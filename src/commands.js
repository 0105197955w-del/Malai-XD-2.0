...
*Made by Kimani Samuel*`;
  ...
  add({ name: 'ban', category: 'owner', ownerOnly: true, desc: 'Ban a user from commands', handler: async (ctx) => {
    ...
    await reply(ctx, `*『 USER BANNED 』*\n\n🚫 *Banned User:* @${normalizeNumber(target)}\n👮 *Banned By:* @${normalizeNumber(ctx.sender)}\n📅 *Date:* ${date}, ${time}`, { mentions: [target, ctx.sender] });
  }});
  add({ name: 'unban', category: 'owner', ownerOnly: true, desc: 'Unban a user', handler: async (ctx) => {
    ...
    saveState(st);
    const { date, time } = getDateTimeParts();
    await reply(ctx, `*『 USER UNBANNED 』*\n\n✅ *Unbanned User:* @${normalizeNumber(target)}\n👮 *Unbanned By:* @${normalizeNumber(ctx.sender)}\n📅 *Date:* ${date}, ${time}`, { mentions: [target, ctx.sender] });
  }});
  ...
  add({ name: 'afk', category: 'utility', desc: 'Set yourself as AFK with an optional reason', usage: '[reason]', handler: async (ctx) => {
    const reason = textArg(ctx.args) || 'AFK';
    const st = getState();
    if (!st.afk) st.afk = {};
    st.afk[ctx.sender] = { reason, since: Date.now() };
    saveState(st);
    await reply(ctx, `💤 You're now AFK: ${reason}`);
  }});
  add({ name: 'poll', category: 'utility', groupOnly: true, desc: 'Create a poll', usage: 'Question | option1 | option2 | ...', handler: async (ctx) => {
    const parts = textArg(ctx.args).split('|').map(s => s.trim()).filter(Boolean);
    if (parts.length < 3) return reply(ctx, 'Usage: poll Question | option1 | option2 | option3...');
    const [question, ...options] = parts;
    if (options.length > 12) return reply(ctx, 'Max 12 options allowed.');
    await ctx.sock.sendMessage(ctx.chatId, { poll: { name: question, values: options, selectableCount: 1 } }, { quoted: ctx.message });
  }});
  add({ name: 'shortlink', aliases: ['short'], category: 'utility', desc: 'Shorten a URL', usage: '<url>', handler: async (ctx) => {
    const url = textArg(ctx.args);
    if (!url || !/^https?:\/\//i.test(url)) return reply(ctx, 'Usage: shortlink https://example.com');
    try {
      const { data } = await axios.get('https://is.gd/create.php', { params: { format: 'simple', url }, timeout: 10000 });
      await reply(ctx, `🔗 ${data}`);
    } catch (e) { await reply(ctx, `Shortlink failed: ${e.message}`); }
  }});
  add({ name: 'currency', aliases: ['conv', 'exchange'], category: 'utility', desc: 'Convert currency', usage: '<amount> <from> <to>', handler: async (ctx) => {
    const [amtRaw, from, to] = ctx.args;
    const amount = Number(amtRaw);
    if (!amount || !from || !to) return reply(ctx, 'Usage: currency 100 USD KES');
    try {
      const { data } = await axios.get(`https://open.er-api.com/v6/latest/${encodeURIComponent(from.toUpperCase())}`, { timeout: 10000 });
      const rate = data?.rates?.[to.toUpperCase()];
      if (!rate) return reply(ctx, `Couldn't find a rate for ${to.toUpperCase()}`);
      const result = (amount * rate).toFixed(2);
      await reply(ctx, `💱 ${amount} ${from.toUpperCase()} = ${result} ${to.toUpperCase()}`);
    } catch (e) { await reply(ctx, `Currency lookup failed: ${e.message}`); }
  }});
  add({ name: 'define', aliases: ['dictionary', 'meaning'], category: 'search', desc: 'Get the definition of a word', usage: '<word>', handler: async (ctx) => {
    const word = textArg(ctx.args);
    if (!word) return reply(ctx, 'Usage: define serendipity');
    try {
      ...
      if (!def) return reply(ctx, `No definition found for "${word}".`);
      const phon = entry.phonetic ? ` ${entry.phonetic}` : '';
      await reply(ctx, `📖 *${word}*${phon}\n_${meaning.partOfSpeech}_\n${def}`);
    } catch (e) { await reply(ctx, `No definition found for "${word}".`); }
  }});
  ...
  add({ name: 'kick', aliases: ['remove'], category: 'group', groupOnly: true, adminOnly: true, desc: 'Remove mentioned/replied users or typed numbers from group', usage: '@user | +254...', handler: async (ctx) => {
    ...
    try {
      ...
      const { date, time } = getDateTimeParts(getConfig().timeZone);
      await reply(ctx, `*『 GROUP KICK 』*\n\n👥 *Removed User(s):*\n${safeTargets.map(j => `• @${normalizeNumber(j)}`).join('\n')}\n👮 *Removed By:* @${normalizeNumber(ctx.sender)}\n📅 *Date:* ${date}, ${time}`, { mentions: [...safeTargets, ctx.sender] });
    } catch (err) {
  ...
  add({ name: 'approve', aliases: ['accept','approveall'], category: 'group', groupOnly: true, adminOnly: true, desc: 'Approve pending group join requests', usage: '[number/@user] or approveall', handler: async (ctx) => {
    ...
    await ctx.sock.groupRequestParticipantsUpdate(ctx.chatId, targets, 'approve');
    const { date, time } = getDateTimeParts();
    await reply(ctx, `*『 JOIN REQUEST APPROVED 』*\n\n✅ *Approved:* ${formatTargetList(targets)}\n👮 *Approved By:* @${normalizeNumber(ctx.sender)}\n📅 *Date:* ${date}, ${time}`, { mentions: [...targets, ctx.sender] });
  }});
    ...
    handler: async (ctx) => {
      ...
      try {
        ...
        if (imgMsg) {
          ...
          await reply(ctx, '*『 STATUS POSTED 』*\n\n✅ Image posted to status successfully!');
        } else if (vidMsg) {
          const stream = await downloadContentFromMessage(vidMsg, 'video');
          const chunks = []; for await (const c of stream) chunks.push(c);
          const buffer = Buffer.concat(chunks);
          ...
          await reply(ctx, '*『 STATUS POSTED 』*\n\n✅ Video posted to status successfully!');
        }
      } catch (err) {
...
*Made by Kimani Samuel*`);
  ...
  add({ name: 'repo', aliases: ['source', 'sourcecode', 'github-repo'], category: 'owner', ownerOnly: true,
    ...
    handler: async (ctx) => {
      ...
      await reply(ctx,
        ...
        `╰━━━━━━━━━━━━━━━━━━╯\n\n` +
        `📦 *GitHub Repository:*\n` +
        `https://github.com/0105197955w-del/Malai-XD-2.0.git\n\n` +
        `🌐 *Browse online:*\n` +
        `https://github.com/0105197955w-del/Malai-XD-2.0\n\n` +
        `📥 *Clone:*\n` +
        `\`git clone https://github.com/0105197955w-del/Malai-XD-2.0.git\`\n\n` +
  ...
  add({ name: 'update', aliases: ['gitpull', 'pullupdate', 'fetchupdate'], category: 'owner', ownerOnly: true,
    ...
    usage: '[force]',
    handler: async (ctx) => {
      const REPO_URL = 'https://github.com/0105197955w-del/Malai-XD-2.0.git';
      ...
      if (!remoteCheck.ok || !remoteCheck.out.includes('0105197955w-del')) {
...
${madeByFooter(cfg)}`;
  ...
  // ─── BIO / AUTOBIO ────────────────────────────────────────────────────────
  if (!registry.has('bio')) add({ name: 'bio', aliases: ['setbio'], category: 'owner', ownerOnly: true, desc: 'Set bot WhatsApp status/bio', usage: '<text>', handler: async (ctx) => {
    const text = textArg(ctx.args);
  ...
  add({ name: 'warn', category: 'group', groupOnly: true, adminOnly: true, desc: 'Warn a user (3 warns = kick)', usage: '@user [reason]', handler: async (ctx) => {
    ...
    for (const jid of targets) {
      ...
      results.push(`*『 GROUP WARNING 』*\n\n⚠️ *Warned User:* @${normalizeNumber(jid)}\n📝 *Reason:* ${reason}\n🔢 *Warnings:* ${count}/3\n👮 *Warned By:* @${normalizeNumber(ctx.sender)}\n📅 *Date:* ${date}, ${time}`);
      if (count >= 3) {
        st.warnings[key] = 0;
        try { await ctx.sock.groupParticipantsUpdate(ctx.chatId, [jid], 'remove'); results.push(`*『 AUTO-KICK 』*\n\n🚫 @${normalizeNumber(jid)} removed after reaching 3 warnings.`); } catch {}
      }
    }
    saveState(st);
  ...
  add({ name: 'resetwarn', aliases: ['clearwarn'], category: 'group', groupOnly: true, adminOnly: true, desc: 'Reset warnings for a user', usage: '@user', handler: async (ctx) => {
    ...
    saveState(st);
    const { date, time } = getDateTimeParts();
    await reply(ctx, `*『 WARNINGS RESET 』*\n\n✅ *User(s):* ${targets.map(j => '@' + normalizeNumber(j)).join(', ')}\n👮 *Reset By:* @${normalizeNumber(ctx.sender)}\n📅 *Date:* ${date}, ${time}`, { mentions: [...targets, ctx.sender] });
  }});
...
${getConfig().prefix}autoread off → Disable`);
  ...
  add({ name: 'botleave', aliases: ['kickbot', 'removebot'], category: 'owner',
    ...
    handler: async (ctx) => {
      try {
        await reply(ctx, '*『 BOT LEAVING 』*\n\n🤖 Malai-XD-2.0 is leaving this group...\n_Goodbye everyone! 👋_');
  ...
  add({ name: 'kickme', aliases: ['removeme', 'exitgroup'], category: 'owner',
    ...
    handler: async (ctx) => {
      try {
        ...
        await reply(ctx, '*『 LEAVING GROUP 』*\n\n👋 Removing you from this group...');
        await new Promise(r => setTimeout(r, 1000));
        await ctx.sock.groupParticipantsUpdate(ctx.chatId, [ownerJid], 'remove');
      } catch (err) {
  ...
  return { commands, registry, getConfig, getState, saveState };
}
